...
#define LIBOFD_SOURCE_DIR "."
    ...
    const fs::path work_dir = src_dir / "tests" / "tmp_layout_pdf";
    std::error_code ec;
    fs::remove_all(work_dir, ec);
    fs::create_directories(work_dir / "Doc_0" / "Pages" / "Page_0", ec);

    // Build an OFD sample with explicit TextObject boundary and size.
    {
    ...
    }
    {
        ...
        doc << "  <ofd:CommonData><ofd:MaxUnitID>2</ofd:MaxUnitID><ofd:PageArea><ofd:PhysicalBox>0 0 210 297</ofd:PhysicalBox></ofd:PageArea></ofd:CommonData>\n";
    ...
    }
    {
        ...
        page << "  <ofd:TextObject ID=\"1\" Boundary=\"10 20 100 10\" Size=\"4\">\n";
        page << "    <ofd:TextCode>Layout Test</ofd:TextCode>\n";
        page << "  </ofd:TextObject>\n";
    ...
    const fs::path pdf_out = work_dir / "layout.pdf";
    ...
    // Expect matrix text placement command for layout-aware rendering.
    if (pdf.find(" Tm\n") == std::string::npos || pdf.find("Layout Test") == std::string::npos) {
        std::cerr << "pdf layout markers not found\n";
        libofd_destroy(h);
        return EXIT_FAILURE;
    }
